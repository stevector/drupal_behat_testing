...
DCI_RUNSCRIPT:     Default is '/opt/phpenv/shims/php RUNNER --php /opt/phpenv/shims/php --url 'http://localhost' --color --concurrency  DCI_CONCURRENCY  --verbose --xml '/var/workspace/results'  DCI_TESTGROUPS  | tee /var/www/html/test.stdout ' "
...
DCI_RUNSCRIPT=${DCI_RUNSCRIPT:-"/opt/phpenv/shims/php ${RUNNER} --php /opt/phpenv/shims/php --url 'http://localhost' --color --concurrency ${DCI_CONCURRENCY} ${VERBO} --xml '/var/workspace/results'"}
